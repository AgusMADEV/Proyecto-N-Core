"""
VERSIÓN 3 - PROCESAMIENTO PARALELO DE IMÁGENES (MULTINÚCLEO)
=============================================================
Este programa procesa múltiples imágenes de forma PARALELA
...
def procesar_imagen(ruta_entrada, ruta_salida, operaciones):
    ...
    print(f"  🔄 [{proceso_id}] Procesando: {nombre_archivo}")
    ...
    try:
        ...
        print(f"  ✅ [{proceso_id}] {nombre_archivo}: "
              f"{tamanio_original} → {tamanio_final} | "
              f"{tamanio_archivo_entrada:.2f} KB → {tamanio_archivo_salida:.2f} KB | "
              f"{tiempo:.3f}s")
        ...
        print(f"  ❌ [{proceso_id}] Error: Archivo no encontrado - {nombre_archivo}")
        return {
            'archivo': nombre_archivo,
            'ruta_entrada': ruta_entrada,
            'exito': False,
        ...
        print(f"  ❌ [{proceso_id}] Error: {str(e)} - {nombre_archivo}")
        return {
            'archivo': nombre_archivo,
            'ruta_entrada': ruta_entrada,
            'exito': False,
...
def procesar_imagenes_paralelo(lista_imagenes, carpeta_salida, operaciones, num_procesos=None):
    """
    Procesa una lista de imágenes de forma PARALELA.
    Distribuye las imágenes entre todos los núcleos disponibles.
    ...
    print("\n" + "="*70)
    print("🚀 INICIANDO PROCESAMIENTO PARALELO DE IMÁGENES")
    print("="*70)
...
def main():
    ...
    """
    print("\n" + "="*70)
    print("🚀 VERSIÓN 3 - PROCESAMIENTO PARALELO DE IMÁGENES")
    print("="*70)
    ...
    for resultado in resultados:
        if resultado['exito']:
            print(f"\n🖼️  {resultado['archivo']} (Proceso: {resultado['proceso']})")
